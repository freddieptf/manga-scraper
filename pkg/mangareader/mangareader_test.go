...
func TestGetMangaReaderMangaList(t *testing.T) {
...
func TestSearchMangaReaderManga(t *testing.T) {
	results, err := searchMangaList(testManga)
	if err != nil {
		t.Fatal(err)
	}
	if len(results) <= 0 {
...
func TestGetMangaChapterPages(t *testing.T) {
	...
	chPages, err := getChPageUrls(chapterList[0].URL)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetImageURLFromChapterPage(t *testing.T) {
	...
	imgURL, err := getChPageImgUrl(chPages[0])
	if err != nil {
		t.Fatal(err)
	}
